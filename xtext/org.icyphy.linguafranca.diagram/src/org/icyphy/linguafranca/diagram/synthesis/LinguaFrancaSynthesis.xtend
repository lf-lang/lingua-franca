...
class LinguaFrancaSynthesis extends AbstractDiagramSynthesis<Model> {
    ...
    override KNode transform(Model model) {
        ...
        model.link // TODO usually Xtext does this
    ...
    private def List<KNode> transformReactorNetwork(Reactor reactor, Map<String,KPort> parentInputPorts, Map<String, KPort> parentOutputPorts) {
    	...
    	for (instance: reactor.instances) {
	        ...
	        node.createReactorFigure() => [
	            ...
	            addText(instance.reactorClass) => [
	                it.fontSize = 11
	                it.setFontBold(true)
	                it.setGridPlacementData().from(LEFT, 8, 0, TOP, 8f, 0).to(RIGHT, 8, 0, BOTTOM, hasContent ? 4 : 8, 0)
	                it.suppressSelectability
	            ]
    		...
    		// Add content
    		if (hasContent) {
    			node.children += clazz.transformReactorNetwork(inputPorts.column(name), outputPorts.column(name))
    	...
    	for (connection : reactor.connections) {
    		...
    		edge.sourcePort = outputPorts.get(left.get(0), left.get(1))
    		edge.source = edge.sourcePort?.node
    		val right = connection.rightPort.split("\\.") // TODO model should better contain cross references
    		edge.targetPort = inputPorts.get(right.get(0), right.get(1))
    		edge.target = edge.targetPort?.node
    		edge.addPolyline.addHeadArrowDecorator
    	}
