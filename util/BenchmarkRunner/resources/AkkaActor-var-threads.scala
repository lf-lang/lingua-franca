...
object AkkaActorState {
  ...
  def initialize(): Unit = {
    ...
    /* [[[cog
      if 'numThreads' in globals():
        cog.outl(f'val corePoolSize = {numThreads}')
        cog.outl(f'val maxPoolSize = {numThreads}')
      else:
        cog.outl('val corePoolSize = getNumWorkers("actors.corePoolSize", 4)')
        cog.outl('val maxPoolSize = getNumWorkers("actors.maxPoolSize", corePoolSize)')
    ]]] */
    val corePoolSize = 16
    val maxPoolSize = 16
    //[[[end]]]
