...
class LspTests {
    ...
    private static final Predicate<List<Diagnostic>> NOT_SUPPORTED =
            diagnosticsHaveKeyword("supported");
    private static final Predicate<List<Diagnostic>> MISSING_DEPENDENCY =
            diagnosticsHaveKeyword("libprotoc")
                    .or(diagnosticsHaveKeyword("protoc-c"))
                    .or(diagnosticsIncludeText("could not be found"));
    /**
     * The number of samples to take from each test category (with replacement) when doing
     * validation tests.
     */
    ...
    private void targetLanguageValidationTest(Target target, ErrorInserter.Builder builder)
        ...
        System.out.printf(
                "Running validation tests for %s with random seed %d.%n",
                target.getDisplayName(), seed);
        ...
        while (i-- > 0)
            checkDiagnostics(
                    target,
                    alteredTest ->
                            MISSING_DEPENDENCY.or(
                                    diagnostics ->
                                            alteredTest.getBadLines().stream()
                                                    .allMatch(
                                                            badLine -> {
                                                                System.out.print(
                                                                        "Expecting an error to be"
                                                                            + " reported at line "
                                                                                + badLine
                                                                                + "...");
                                                                boolean result =
     ...
     * @param alterer The means of inserting problems into the tests, or {@code null} if problems
            ...
            if (alterer != null) {
                try (AlteredTest altered = alterer.alterTest(test.getSrcPath())) {
                    ...
                    Assertions.assertTrue(
                            requirementGetter.apply(altered).test(client.getReceivedDiagnostics()));
                }
            } else {
                runTest(test.getSrcPath());
                Assertions.assertTrue(
                        requirementGetter.apply(null).test(client.getReceivedDiagnostics()));
            }
        }
    }
    ...
    private Iterable<? extends TestCategory> selectedCategories() {
        return () ->
                Arrays.stream(TestCategory.values())
                        .filter(
                                category ->
                                        Arrays.stream(EXCLUDED_CATEGORIES)
                                                .noneMatch(category::equals))
                        .iterator();
    }
    ...
    private static Predicate<List<Diagnostic>> diagnosticsHaveKeyword(String keyword) {
        return diagnostics ->
                diagnostics.stream()
                        .anyMatch(
                                d ->
                                        Arrays.asList(d.getMessage().toLowerCase().split("\\b"))
                                                .contains(keyword));
    }
    ...
    private static Predicate<List<Diagnostic>> diagnosticsIncludeText(
        ...
        return diagnostics ->
                diagnostics.stream()
                        .anyMatch(d -> d.getMessage().toLowerCase().contains(requiredText));
    }
