...
class LFProjectTemplateProvider implements IProjectTemplateProvider {
	...
	static def getTemplateFromFile(AbstractProjectTemplate tpl, String target, String fileName) {
        val stream = tpl.getClass().getResourceAsStream("templates/" + target + "/" + fileName);
        var str = ""
        if (stream !== null) {
    ...
    final class FederatedProject {
        ...
        override generateProjects(IProjectGenerator generator) {
            generator.generate(new PluginProjectFactory => [
                ...
                builderIds += #[XtextProjectHelper.BUILDER_ID]
                folders += #["src"]
                val fileName = "src/FederatedHelloWorld.lf"
                addFile(fileName, LFProjectTemplateProvider.getTemplateFromFile(this, "c", fileName))
            ])
        }
    }
