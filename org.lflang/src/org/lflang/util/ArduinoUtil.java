...
public class ArduinoUtil {
    ...
    private LFCommand arduinoCompileCommand(FileConfig fileConfig, TargetConfig targetConfig)
        ...
        if (!checkArduinoCLIExists()) {
            ...
            try {
                // Write to a temporary file to execute since ProcessBuilder does not like spaces
                // and double quotes in its arguments.
                ...
                bufferedWriter.write(
                        "arduino-cli compile -b "
                                ...
                                + " -DINITIAL_REACT_QUEUE_SIZE=10\" --build-property"
    ...
    public void buildArduino(FileConfig fileConfig, TargetConfig targetConfig) {
        ...
        try {
            LFCommand command =
                    arduinoCompileCommand(
                            fileConfig, targetConfig); // Use ProcessBuilder for finer control.
            int retCode = 0;
        ...
        if (targetConfig.platformOptions.flash) {
            if (targetConfig.platformOptions.port != null) {
                ...
                errorReporter.reportError(
                        "Need to provide a port on which to automatically flash.");
            }
        } else {
            System.out.println("********");
            System.out.println(
                    "To flash your program, run the following command to see information about the"
                        + " board you plugged in:\n\n"
                        + "\tarduino-cli board list\n\n"
                        + "Grab the FQBN and PORT from the command and run the following command in"
