...
public class CollectionUtil {
    ...
    /**
     * Returns a set which contains the elements of the given set plus the given element. The
     * returned set should be considered unmodifiable.
     *
     ...
     * graph. There, we use maps to represent edges, where a value in a map is a set of nodes
     * adjacent to the key for that value. Hence, when a node is removed, it needs to be removed not
     * just as a key, but it also needs to be removed from the neighbors sets of any other keys that
     * may contain it.
     *
    ...
    /**
     * Apply a transform to the values of the map. If the mapping function returns null, the entry
     * is removed.
     */
     ...
     * value (null if the key is not present) as inputs, and returns the new value to associate to
     * the key (null if the mapping should be removed).
     *
