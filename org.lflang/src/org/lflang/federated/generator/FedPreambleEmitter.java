...
public class FedPreambleEmitter {
    ...
    String generatePreamble(
        ...
        var mainModel = (Model)
                ASTUtils.toDefinition(federate.instantiation.getReactorClass()).eContainer();
        for (Preamble p : mainModel.getPreambles()) {
            ...
            =}
            """
                    .formatted(p.getVisibility() == null ? "" : p.getVisibility() + " ", toText(p.getCode())));
        }
        ...
        preambleCode.pr("""
            ...
            %s
            =}"""
                .formatted(FedTargetExtensionFactory.getExtension(federate.targetConfig.target)
