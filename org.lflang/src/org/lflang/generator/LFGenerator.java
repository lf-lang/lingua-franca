...
public class LFGenerator extends AbstractGenerator {
     ...
     * Application", the Kotlin classes are unfortunately not available at runtime due to bugs in
     * the Eclipse Kotlin plugin. (See
     ...
     *
     * <p>If the FileConfig class is found, this method returns an instance. Otherwise, it returns
                        ...
                        return (FileConfig)
                                Class.forName(className)
                                        .getDeclaredConstructor(
                                                Resource.class, Path.class, boolean.class)
    ...
    private GeneratorBase createGenerator(LFGeneratorContext context) {
        final Target target =
                Target.fromDecl(ASTUtils.targetDecl(context.getFileConfig().resource));
        assert target != null;
     ...
     *
     * <p>Since the CppGenerator and TSGenerator class are implemented in Kotlin, the classes are
     * not visible from all contexts. If the RCA is run from within Eclipse via "Run as Eclipse
     * Application", the Kotlin classes are unfortunately not available at runtime due to bugs in
     * the Eclipse Kotlin plugin. (See
     * https://stackoverflow.com/questions/68095816/is-ist-possible-to-build-mixed-kotlin-and-java-applications-with-a-recent-eclips)
            ...
            context.getErrorReporter()
                    .reportError(
                            "The code generator for the "
                                    ...
                                    + "Eclipse. The "
                                    + target
                                    + " code generator is written in Kotlin and, unfortunately, the"
                                    + " plugin that Eclipse uses for compiling Kotlin code is"
                                    + " broken. Please consider building Epoch using Maven.\n"
        ...
        if (FedASTUtils.findFederatedReactor(resource) != null) {
            try {
                generatorErrorsOccurred =
                        (new FedGenerator(lfContext)).doGenerate(resource, lfContext);
            } catch (IOException e) {
