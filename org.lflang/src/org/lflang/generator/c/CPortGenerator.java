...
public class CPortGenerator {
    ...
    public static String generateAuxiliaryStruct(
        ...
        code.unindent();
        var name = decl != null ? localPortName(decl, port.getName()) : variableStructType(port, r, userFacing);
        code.pr("} " + name + ";");
        ...
        if (input.isMultiport()) {
            ...
            if (AttributeUtils.isSparse(input.getDefinition())) {
                return String.join(
                        ...
                        "            sizeof(lf_sparse_io_record_t) + sizeof(size_t) * " + input.getWidth()
                                + "/LF_SPARSE_CAPACITY_DIVIDER,",
                        "            &" + reactorSelfStruct + "->base.allocations);",
                        "    " + portRefName + "__sparse->capacity = " + input.getWidth()
                                + "/LF_SPARSE_CAPACITY_DIVIDER;",
     ...
     */
    private static String valueDeclaration(Port port, Target target, ErrorReporter errorReporter, CTypes types) {
