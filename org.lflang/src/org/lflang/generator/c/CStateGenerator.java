...
public class CStateGenerator {
     ...
     * Otherwise, do, because static initializers for arrays and structs have to be handled this
     * way, and there is no way to tell whether the type of the array is a struct.
     *
     * @param instance
     * @param stateVar
     * @param mode
    ...
    private static String generateBaseInitializer(
            String selfRef, StateVar stateVar, String initExpr, CTypes types) {
        if (ASTUtils.isOfTimeType(stateVar)
            ...
            var declaration =
                    types.getVariableDeclaration(
                            ASTUtils.getInferredType(stateVar), "_initial", true);
            return String.join(
                    "\n",
                    "{ // For scoping",
    ...
    private static String generateModalReset(
        ...
        if (ASTUtils.isOfTimeType(stateVar)
            ...
            var source = "_initial";
            var declaration =
                    types.getVariableDeclaration(ASTUtils.getInferredType(stateVar), source, true);
            code.pr("{ // For scoping");
            code.indent();
