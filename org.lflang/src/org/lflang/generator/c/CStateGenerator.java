...
public class CStateGenerator {
    ...
    public static String generateInitializer(
            ReactorInstance instance, String selfRef, StateVar stateVar, ModeInstance mode, CTypes types) {
    ...
    private static String generateBaseInitializer(String selfRef, StateVar stateVar, String initExpr, CTypes types) {
        if (ASTUtils.isOfTimeType(stateVar)
            ...
            var declaration = types.getVariableDeclaration(ASTUtils.getInferredType(stateVar), "_initial", true);
            return String.join(
                    "\n",
                    "{ // For scoping",
    ...
    private static String generateModalReset(
        ...
        var modeRef = "&" + CUtil.reactorRef(mode.getParent()) + "->_lf__modes["
        ...
        if (ASTUtils.isOfTimeType(stateVar)
                || ASTUtils.isParameterized(stateVar)
                        && !stateVar.getInit().getExprs().isEmpty()) {
            return CModesGenerator.generateStateResetStructure(modeRef, selfRef, stateVar.getName(), initExpr, type);
        } else {
            CodeBuilder code = new CodeBuilder();
            var source = "_initial";
            var declaration = types.getVariableDeclaration(ASTUtils.getInferredType(stateVar), source, true);
            code.pr("{ // For scoping");
            code.indent();
            code.pr("static " + declaration + " = " + initExpr + ";");
            code.pr(CModesGenerator.generateStateResetStructure(modeRef, selfRef, stateVar.getName(), source, type));
            code.unindent();
            code.pr("} // End scoping.");
            return code.toString();
        }
