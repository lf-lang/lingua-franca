...
public class CModesGenerator {
            ...
            for (int i = 0; i < allModes.size(); i++) {
                ...
                constructorCode.pr(
                        ...
                        String.join(
                                ...
                                "self->_lf__modes["
                                        + i
                                        + "].state = &_lf_self_base->_lf__mode_state;",
    ...
    public static String generateModeStatesTable(
        ...
        if (hasModalReactors) {
            return String.join(
                    "\n",
                    "// Array of pointers to mode states to be handled in"
                    ...
                    (modalStateResetCount > 0
                            ? String.join(
                                    "\n",
                                    "// Array of reset data for state variables nested in modes."
                                            + " Used in _lf_handle_mode_changes().",
                                    "mode_state_variable_reset_data_t _lf_modal_state_reset["
                                            + modalStateResetCount
                                            + "];",
                                    "int _lf_modal_state_reset_size = "
                                            + modalStateResetCount
                                            + ";")
                            : ""));
        }
    ...
    public static void generateModeStructure(ReactorInstance instance, CodeBuilder code) {
        ...
        if (parentMode != null) {
            ...
            // If this reactor does not have its own modes, all reactions must be linked to
            // enclosing mode
            if (instance.modes.isEmpty()) {
            ...
            } else { // Otherwise, only reactions outside modes must be linked and the mode state
                // itself gets a parent relation
                code.pr(
                        "((self_base_t*)"
                                + nameOfSelfStruct
                ...
                for (var reaction :
                        (Iterable<ReactionInstance>)
                                instance.reactions.stream().filter(it -> it.getMode(true) == null)
                                        ::iterator) {
                    code.pr(
                            CUtil.reactorRef(reaction.getParent())
                                    + "->_lf__reaction_"
