...
public class CModesGenerator {
    ...
    public static void generateModeStructure(ReactorInstance instance, CodeBuilder code) {
        ...
        if (parentMode != null) {
            var parentModeRef = "&" + CUtil.reactorRef(parentMode.getParent()) + "->_lf__modes["
            ...
            } else { // Otherwise, only reactions outside modes must be linked and the mode state itself gets a parent
                // relation
                code.pr("((self_base_t*)" + nameOfSelfStruct + ")->_lf__mode_state.parent_mode = " + parentModeRef
                        + ";");
                for (var reaction : (Iterable<ReactionInstance>)
                        instance.reactions.stream().filter(it -> it.getMode(true) == null)::iterator) {
                    code.pr(CUtil.reactorRef(reaction.getParent()) + "->_lf__reaction_"
                            + instance.reactions.indexOf(reaction) + ".mode = " + parentModeRef + ";");
                }
            }
        }
        ...
        if (!instance.modes.isEmpty()) {
            ...
            code.pr("_lf_modal_reactor_states[_lf_modal_reactor_states_count++] = &((self_base_t*)" + nameOfSelfStruct
                    + ")->_lf__mode_state;");
        }
    }
     ...
     * @param modalStateResetCount The number of modal model state resets
     */
    public static String generateLfHandleModeChanges(boolean hasModalReactors, int modalStateResetCount) {
        if (!hasModalReactors) {
            return "";
        }
        return String.join(
