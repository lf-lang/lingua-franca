...
public class CReactorHeaderFileGenerator {
    ...
    public static Path outputPath(Reactor r) {
        return Path.of(
                        Path.of(r.eResource().getURI().toFileString())
                                .getFileName()
                                .toString()
                                .replaceFirst("[.][^.]+$", ""))
                .resolve(r.getName() + ".h");
    ...
    private static void appendSignature(CodeBuilder builder, Reaction r, Reactor reactor) {
        if (r.getName() != null)
            builder.pr("void " + r.getName() + "(" + reactionParameters(r, reactor) + ");");
    }
    ...
    private static Stream<PortVariable> portVariableStream(
            Reaction r, Reactor defaultReactorClass) {
