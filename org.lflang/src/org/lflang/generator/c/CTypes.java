...
public class CTypes implements TargetTypes {
    ...
    /**
     * Given a type, return a C representation of the type. Note that C types are very
     * idiosyncratic. For example, {@code int[]} is not always accepted as a type, and {@code int*}
     * must be used instead, except when initializing a variable using a static initializer, as in
     * {@code int[] foo = {1, 2, 3};}. When initializing a variable using a static initializer, use
     ...
     * over the pointer syntax {@code *} (if applicable). This also includes the variable name
     * because C requires the array type specification to be placed after the variable name rather
     * than with the type. The result is of the form {@code type variable_name[size]} or {@code type
     * variable_name} depending on whether the given type is an array type, unless the array type
     * has no size (it is given as {@code []}. In that case, the returned form depends on the third
    ...
    public String getVariableDeclaration(
            InferredType type, String variableName, boolean initializer) {
        ...
        if (matcher.find()) {
            ...
            if (matcher.group(2).equals("") && !initializer) {
                ...
                declaration =
                        String.format(
                                "%s %s[%s]", matcher.group(1), variableName, matcher.group(2));
            }
        }
        return declaration;
    }
