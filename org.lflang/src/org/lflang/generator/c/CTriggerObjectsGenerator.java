...
public class CTriggerObjectsGenerator {
        ...
        if (startTimeStepIsPresentCount > 0) {
            ...
            code.pr(String.join(
                    ...
                    "if (_lf_is_present_fields == NULL) lf_print_error_and_exit(" + addDoubleQuotes("Out of memory!")
                            + ");",
                    "_lf_is_present_fields_abbreviated = (bool**)calloc(" + startTimeStepIsPresentCount
                            + ", sizeof(bool*));",
     ...
     */
    public static String generateSchedulerInitializer(ReactorInstance main, TargetConfig targetConfig) {
        ...
        for (ReactionInstance r : reactor.reactions) {
            ...
            if (levelSet.size() == 1 && deadlineSet.size() == 1) {
                ...
                temp.pr(String.join(
                        ...
                        CUtil.reactionRef(r) + ".index = (" + r.uniqueID() + "_inferred_deadlines[" + runtimeIdx
                                + "] << 16) | " + level + ";"));
                ...
                temp.pr(String.join(
                        ...
                        "// deadlines[" + runtimeIdx + "] shifted left 16 bits.",
                        CUtil.reactionRef(r) + ".index = (" + r.uniqueID() + "_inferred_deadlines[" + runtimeIdx
                                + "] << 16) | " + r.uniqueID() + "_levels[" + runtimeIdx + "];"));
            }
        }
        ...
        for (SendRange srcRange : src.eventualDestinations()) {
            for (RuntimeRange<PortInstance> dstRange : srcRange.destinations) {
                ...
                if (src.isInput()) {
                    ...
                    code.pr(CUtil.portRef(dst, dr, db, dc) + " = (" + destStructType + "*)" + mod
                            + CUtil.portRefNested(src, sr, sb, sc) + ";");
                } else if (dst.isOutput()) {
                    // An output port of a contained reactor is triggering a reaction.
                    code.pr(CUtil.portRefNested(dst, dr, db, dc) + " = (" + destStructType + "*)&"
                            + CUtil.portRef(src, sr, sb, sc) + ";");
                } else {
                    // An output port is triggering an input port.
                    code.pr(CUtil.portRef(dst, dr, db, dc) + " = (" + destStructType + "*)&"
                            + CUtil.portRef(src, sr, sb, sc) + ";");
                    if (AttributeUtils.isSparse(dst.getDefinition())) {
                        code.pr(CUtil.portRef(dst, dr, db, dc) + "->sparse_record = "
                                + CUtil.portRefName(dst, dr, db, dc) + "__sparse;");
        ...
        for (ReactionInstance reaction : reactions) {
            ...
            for (PortInstance port : Iterables.filter(reaction.effects, PortInstance.class)) {
                if (!foundPort) {
                    ...
                    code.startScopedBlock();
                    code.pr("int triggers_index[" + reaction.getParent().getTotalWidth()
                ...
                for (SendRange srcRange : port.eventualDestinations()) {
                    ...
                    code.pr(String.join(
                            "\n",
                            "// Reaction " + reaction.index + " of " + name + " triggers "
                                    + srcRange.destinations.size() + " downstream reactions",
                            "// through port " + port.getFullName() + ".",
                            CUtil.reactionRef(reaction, sr) + ".triggered_sizes[triggers_index[" + sr + "]] = "
            ...
            for (PortInstance port : Iterables.filter(reaction.effects, PortInstance.class)) {
                ...
                code.pr("for (int i = 0; i < " + reaction.getParent().getTotalWidth() + "; i++) triggers_index[i] = "
                        + cumulativePortWidth + ";");
                for (SendRange srcRange : port.eventualDestinations()) {
                    ...
                    for (RuntimeRange<PortInstance> dstRange : srcRange.destinations) {
                        ...
                        if (srcNested && port.getParent().getWidth() > 1) {
                            triggerArray = CUtil.reactionRef(reaction, sr) + ".triggers[triggers_index[" + sr + "] + "
                                    + sc + " + src_range_mr.digits[1] * src_range_mr.radixes[0]]";
                        } else {
                            triggerArray = CUtil.reactionRef(reaction, sr) + ".triggers[triggers_index[" + sr + "] + "
                                    + sc + "]";
                        }
     ...
     */
    private static String deferredInputNumDestinations(Iterable<ReactionInstance> reactions, CTypes types) {
        ...
        for (ReactionInstance reaction : reactions) {
            for (PortInstance port : Iterables.filter(reaction.effects, PortInstance.class)) {
                if (port.isInput() && !portsHandled.contains(port)) {
                    ...
                    code.pr("// Set number of destination reactors for port "
                            + port.getParent().getName() + "." + port.getName() + ".");
                    ...
                    for (SendRange sendingRange : port.eventualDestinations()) {
                        ...
                        code.pr(CUtil.portRefNested(port, sr, sb, sc) + connector + "num_destinations = "
    ...
    private static String deferredOutputNumDestinations(ReactorInstance reactor) {
        ...
        for (PortInstance output : reactor.outputs) {
            for (SendRange sendingRange : output.eventualDestinations()) {
                ...
                code.startScopedRangeBlock(sendingRange, sr, sb, sc, sendingRange.instance.isInput());
                code.pr(CUtil.portRef(output, sr, sb, sc) + ".num_destinations = "
                        + sendingRange.getNumberOfDestinationReactors() + ";");
                code.endScopedRangeBlock(sendingRange);
            }
        }
    ...
    private static String deferredInitializeNonNested(
            ReactorInstance reactor, ReactorInstance main, Iterable<ReactionInstance> reactions, CTypes types) {
        var code = new CodeBuilder();
     ...
     */
    private static String deferredReactionOutputs(ReactionInstance reaction, TargetConfig targetConfig) {
        var code = new CodeBuilder();
        ...
        code.pr(String.join(
                "\n", init.toString(), "// ** End initialization for reaction " + reaction.index + " of " + name));
        return code.toString();
    }
     ...
     */
    private static String deferredReactionMemory(Iterable<ReactionInstance> reactions, TargetConfig targetConfig) {
        var code = new CodeBuilder();
    ...
    private static String deferredInitialize(
            ReactorInstance reactor, Iterable<ReactionInstance> reactions, TargetConfig targetConfig, CTypes types) {
        var code = new CodeBuilder();
