...
public class CTriggerObjectsGenerator {
    ...
    public static String generateInitializeTriggerObjects(
        ...
        if (startTimeStepIsPresentCount > 0) {
            ...
            code.pr(
                    String.join(
                            "\n",
                            "// Create the array that will contain pointers to is_present fields to"
                                    + " reset on each step.",
                            ...
                            "_lf_is_present_fields_abbreviated = (bool**)calloc("
                                    + startTimeStepIsPresentCount
                                    + ", sizeof(bool*));",
                            "if (_lf_is_present_fields_abbreviated == NULL)"
    ...
    public static String generateSchedulerInitializer(
        ...
        var numReactionsPerLevelJoined =
                Arrays.stream(numReactionsPerLevel)
                        .map(String::valueOf)
    ...
    /**
     * Generate assignments of pointers in the "self" struct of a destination port's reactor to the
     * appropriate entries in the "self" struct of the source reactor. If this port is an input,
     * then it is being written to by a reaction belonging to the parent of the port's parent. If it
     * is an output, then it is being written to by a reaction belonging to the port's parent.
     *
    ...
    private static String deferredOptimizeForSingleDominatingReaction(ReactorInstance r) {
        ...
        for (ReactionInstance reaction : r.reactions) {
            ...
            var domStart = 0;
            var same =
                    false; // Set to true when finding a string of identical dominating reactions.
                ...
                if (!first) { // Not the first time through the loop.
                    if (same) { // Previously seen at least two identical dominating.
                        ...
                        same = true;
                            ...
                            && runtime.dominating.getReaction()
                                    == previousRuntime.dominating.getReaction()) {
            ...
            if (runtime.dominating != null) {
                if (same) {
                    dominatingRef =
                            "&("
                                    + CUtil.reactionRef(
                                            runtime.dominating.getReaction(), "" + domStart)
                                    + ")";
                } else {
                    dominatingRef =
                            "&(" + CUtil.reactionRef(runtime.dominating.getReaction(), "j++") + ")";
                }
            }
            code.pr(
                    String.join(
                            "\n",
                            "// "
                                    + runtime.getReaction().getFullName()
                                    + " dominating upstream reaction.",
                            "int j = " + domStart + ";",
    ...
    /**
     * For the specified reaction, for ports that it writes to, fill the trigger table for
                    ...
                    code.pr(
                            String.join(
                                    ...
                                    "// For reaction "
                                            ...
                                            + name
                                            + ", allocate an",
                                    "// array of trigger pointers for downstream reactions through"
                                            + " port "
                                            + port.getFullName(),
            ...
            for (PortInstance port : Iterables.filter(reaction.effects, PortInstance.class)) {
                ...
                for (SendRange srcRange : port.eventualDestinations()) {
                    ...
                    for (RuntimeRange<PortInstance> dstRange : srcRange.destinations) {
                        ...
                        // If the source is nested, need to take into account the parent's bank
                        // index
                        ...
                        if (srcNested && port.getParent().getWidth() > 1) {
                            triggerArray =
                                    CUtil.reactionRef(reaction, sr)
                                            ...
                                            + "] + "
                                            + sc
                                            + " + src_range_mr.digits[1] *"
                                            + " src_range_mr.radixes[0]]";
                        } else {
                            triggerArray =
                                    CUtil.reactionRef(reaction, sr)
                                            + ".triggers[triggers_index["
                        ...
                        if (dst.isOutput()) {
                            ...
                            var belongs = false;
                            for (ReactionInstance destinationReaction :
                                    dst.getDependentReactions()) {
                                belongs = true;
                            }
                            if (belongs) {
                                code.pr(
                                        String.join(
                                                ...
                                                "// Port "
                                                        + port.getFullName()
                                                        + " has reactions in its parent's parent.",
                                                "// Point to the trigger struct for those"
                                                        + " reactions.",
                                                triggerArray
                                                        + "["
                                                        + multicastCount
    ...
    /**
     * For each input port of a contained reactor that receives data from one or more of the
     * specified reactions, set the num_destinations field of the corresponding port structs on the
     * self struct of the reaction's parent reactor equal to the total number of destination
     * reactors. If the port has a token type, this also initializes it with a token.
     *
     * @param reactions The reactions.
        ...
        for (ReactionInstance reaction : reactions) {
            for (PortInstance port : Iterables.filter(reaction.effects, PortInstance.class)) {
                if (port.isInput() && !portsHandled.contains(port)) {
                    // Port is an input of a contained reactor that gets data from a reaction of
                    // this reactor.
                    ...
                    for (SendRange sendingRange : port.eventualDestinations()) {
                        code.startScopedRangeBlock(
                                sendingRange, sr, sb, sc, sendingRange.instance.isInput());
                        ...
                        if (CUtil.isTokenType(type, types)) {
                            ...
                            // If the port is a multiport, then the portRefNested is itself a
                            // pointer
     ...
     * actually produced. The port may be an output of the reaction's parent or an input to a
     * reactor contained by the parent.
     *
    ...
    private static String deferredReactionOutputs(
        ...
        if (outputCount > 0) {
            code.pr(
                    String.join(
                            "\n",
                            "// Allocate memory for triggers[] and triggered_sizes[] on the"
                                    + " reaction_t",
                            ...
                            "        &" + reactorSelfStruct + "->base.allocations);",
                            CUtil.reactionRef(reaction)
                                    + ".output_produced = (bool**)_lf_allocate(",
     ...
     * code to allocate memory to store the data produced by those reactions. The allocated memory
     * is pointed to by a field called `_lf_containername.portname` on the self struct of the
     * reactor.
     *
     * @param reactor The reactor.
     */
        ...
        for (ReactionInstance reaction : reactor.reactions) {
            for (PortInstance effect : Iterables.filter(reaction.effects, PortInstance.class)) {
                if (effect.getParent().getDepth()
                                > reactor.getDepth() // port of a contained reactor.
                        && effect.isMultiport()
