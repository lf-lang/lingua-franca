...
public interface TargetTypes {
    ...
    /**
     * Returns an expression in the target language that corresponds to a fixed-size list
     * expression.
     *
     * @throws UnsupportedGeneratorFeatureException If the target does not support this
     */
    ...
    /**
     * Returns the expression that is used to replace a missing expression in the source language.
     * The expression may for instance be a type-agnostic default value (e.g. Rust's {@code
     * Default::default()}), or produce a compiler error (e.g. Rust's {@code
     * compiler_error!("missing initializer")}).
     *
     * @throws UnsupportedGeneratorFeatureException If the target does not support this
     */
    ...
    /**
     * Returns a target type inferred from the type node, or the initializer list. If both are
     * absent, then the undefined type is returned.
     */
    ...
    /**
     * Returns the representation of the given initializer expression in target code. The given
     * type, if non-null, may inform the code generation.
     *
    ...
    default String getTargetInitializer(Initializer init, Type type) {
        ...
        if (inferredType.isFixedSizeList) {
            return getFixedSizeListInitExpression(
                    targetValues, inferredType.listSize, init.isBraces());
        } else {
    ...
    /**
     * Returns the representation of the given expression in target code. The given type, if
     * non-null, may inform the code generation.
     */
    default String getTargetExpr(Expression expr, InferredType type) {
        if (ASTUtils.isZero(expr) && type != null && type.isTime) {
            ...
            return ASTUtils.addZeroToLeadingDot(
                    ((Literal) expr).getLiteral()); // here we don't escape
