...
public class PythonReactionGenerator {
     ...
     * @param pyObjects CPython related objects
     */
    public static String generateCPythonDeadlineCaller(Reactor r, int reactionIndex, List<String> pyObjects) {
     ...
     * @param pyObjects CPython related objects
     */
    public static String generateCPythonSTPCaller(Reactor r, int reactionIndex, List<String> pyObjects) {
    ...
    private static String generateCPythonFunctionCaller(
        ...
        code.pr(String.join(
                ...
                ");",
                "if (rValue == NULL) {",
                "    lf_print_error(\"FATAL: Calling reaction " + reactorDeclName + "." + pythonFunctionName
                        + " failed.\");",
                "    if (PyErr_Occurred()) {",
                "        PyErr_PrintEx(0);",
    ...
    public static String generateCReaction(
        ...
        String cInit = CReactionGenerator.generateInitializationForReaction(
                "", reaction, r, reactionIndex, types, errorReporter, mainDef, Target.Python.requiresTypes);
        ...
        if (reaction.getEffects() != null) {
            for (VarRef effect : reaction.getEffects()) {
                if (effect.getVariable() instanceof Action) {
                    ...
                    pyObjects.add(
                            "convert_C_mode_to_py(" + name + ",(self_base_t*)self, _lf_" + name + "_change_type)");
                } else {
                    if (effect.getVariable() instanceof Output) {
                        ...
                        code.pr(PythonPortGenerator.generateVariablesForSendingToContainedReactors(
                                pyObjects, effect.getContainer(), (Input) effect.getVariable()));
                    } else {
                        errorReporter.reportError(
                                reaction,
                                "In generateReaction(): " + effect.getVariable().getName()
    ...
    public static void generatePythonReactionParametersAndInitializations(
                ...
                if (triggerAsVarRef.getVariable() instanceof Input) {
                    if (((Input) triggerAsVarRef.getVariable()).isMutable()) {
                        ...
                        if (ASTUtils.isMultiport((Input) triggerAsVarRef.getVariable())) {
                            inits.pr(triggerAsVarRef.getVariable().getName() + " = [Make() for i in range(len(mutable_"
                                    + triggerAsVarRef.getVariable().getName() + "))]");
                            inits.pr("for i in range(len(mutable_"
                                    + triggerAsVarRef.getVariable().getName() + ")):");
                            inits.pr("    " + triggerAsVarRef.getVariable().getName()
                                    + "[i].value = copy.deepcopy(mutable_"
                                    + triggerAsVarRef.getVariable().getName() + "[i].value)");
                        } else {
                            inits.pr(triggerAsVarRef.getVariable().getName() + " = Make()");
                            inits.pr(triggerAsVarRef.getVariable().getName() + ".value = copy.deepcopy(mutable_"
                                    + triggerAsVarRef.getVariable().getName() + ".value)");
                        }
                    } else {
                    ...
                    generatedParams.add(triggerAsVarRef.getContainer().getName() + "_"
                            + triggerAsVarRef.getVariable().getName());
            ...
            if (src.getVariable() instanceof Output) {
                ...
                if (src.getVariable() instanceof Input) {
                    if (((Input) src.getVariable()).isMutable()) {
                        // Create a deep copy
                        inits.pr(src.getVariable().getName() + " = copy.deepcopy("
        ...
        for (VarRef effect : ASTUtils.convertToEmptyListIfNull(reaction.getEffects())) {
            if (effect.getVariable() instanceof Input) {
                generatedParams.add(effect.getContainer().getName() + "_"
    ...
    private static String generateVariableToSendPythonReaction(
            VarRef varRef, Set<Action> actionsAsTriggers, ReactorDecl decl, List<String> pyObjects) {
     ...
     */
    public static String generateCPythonReactionLinkers(ReactorInstance instance, Instantiation mainDef) {
    ...
    private static String generateCPythonFunctionLinker(
            String nameOfSelfStruct, String cpythonFunctionName, ReactorInstance instance, String pythonFunctionName) {
        return String.join(
                "\n",
    ...
    public static String generatePythonFunction(
            String pythonFunctionName, String inits, String reactionBody, List<String> reactionParameters) {
