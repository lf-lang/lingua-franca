...
public class PythonReactionGenerator {
    ...
    public static String generateCPythonReactionCaller(
            Reactor reactor, int reactionIndex, List<String> pyObjects, String inits) {
        ...
        return generateCPythonFunctionCaller(
                CUtil.getName(reactor), pythonFunctionName, cpythonFunctionName, pyObjects, inits);
    }
     ...
     * @param pyObjects CPython related objects
     */
    public static String generateCPythonDeadlineCaller(
            Reactor r, int reactionIndex, List<String> pyObjects) {
        ...
        return generateCPythonFunctionCaller(
                CUtil.getName(r), pythonFunctionName, cpythonFunctionName, pyObjects, "");
    }
     ...
     * @param pyObjects CPython related objects
     */
    public static String generateCPythonSTPCaller(
            Reactor r, int reactionIndex, List<String> pyObjects) {
     ...
     * @param pyObjects CPython related objects
     */
    private static String generateCPythonFunctionCaller(
            String reactorDeclName,
        ...
        code.pr(
                String.join(
                        "\n",
                        "LF_PRINT_DEBUG(\"Calling reaction function "
                                + reactorDeclName
                                + "."
                                + pythonFunctionName
                                + "\");",
                        ...
                        "    if (PyErr_Occurred()) {",
                        "        PyErr_PrintEx(0);",
                        "        PyErr_Clear(); // this will reset the error indicator so we can"
    ...
    public static String generateFunction(String header, String init, Code code, String pyCaller) {
    ...
    private static String generateCPythonInitializers(
        ...
        for (TriggerRef trigger : ASTUtils.convertToEmptyListIfNull(reaction.getTriggers())) {
            if (trigger instanceof VarRef) {
                ...
                code.pr(
                        generateVariableToSendPythonReaction(
                                triggerAsVarRef, actionsAsTriggers, decl, pyObjects));
            }
        }
        if (reaction.getTriggers() == null || reaction.getTriggers().size() == 0) {
            ...
            for (Input input : reactor.getInputs()) {
                PythonPortGenerator.generateInputVariablesToSendToPythonReaction(
                        pyObjects, input, decl);
            }
        }
            ...
            for (VarRef effect : reaction.getEffects()) {
                if (effect.getVariable() instanceof Action) {
                    ...
                    if (!actionsAsTriggers.contains(effect.getVariable())) {
                        PythonPortGenerator.generateActionVariableToSendToPythonReaction(
                                pyObjects, (Action) effect.getVariable(), decl);
                    }
                    ...
                    pyObjects.add(
                            "convert_C_mode_to_py("
                                    + name
                                    + ",(self_base_t*)self, _lf_"
                                    + name
                                    + "_change_type)");
                } else {
                    if (effect.getVariable() instanceof Output) {
                        PythonPortGenerator.generateOutputVariablesToSendToPythonReaction(
                                pyObjects, (Output) effect.getVariable());
    ...
    /**
     * Generate parameters and their respective initialization code for a reaction function The
     ...
     */
    public static void generatePythonReactionParametersAndInitializations(
            List<String> parameters, CodeBuilder inits, ReactorDecl decl, Reaction reaction) {
        Reactor reactor = ASTUtils.toDefinition(decl);
            ...
            if (triggerAsVarRef.getVariable() instanceof Port) {
                if (triggerAsVarRef.getVariable() instanceof Input) {
                    if (((Input) triggerAsVarRef.getVariable()).isMutable()) {
                        generatedParams.add(
                                "mutable_" + triggerAsVarRef.getVariable().getName() + "");

                        // Create a deep copy
                        if (ASTUtils.isMultiport((Input) triggerAsVarRef.getVariable())) {
                            inits.pr(
                                    triggerAsVarRef.getVariable().getName()
                                            + " = [Make() for i in range(len(mutable_"
                                            + triggerAsVarRef.getVariable().getName()
                                            + "))]");
                            inits.pr(
                                    "for i in range(len(mutable_"
                                            + triggerAsVarRef.getVariable().getName()
                                            + ")):");
                            inits.pr(
                                    "    "
                                            + triggerAsVarRef.getVariable().getName()
                                            + "[i].value = copy.deepcopy(mutable_"
                                            + triggerAsVarRef.getVariable().getName()
                                            + "[i].value)");
                        } else {
                    ...
                    generatedParams.add(
                            triggerAsVarRef.getContainer().getName()
                                    + "_"
                                    + triggerAsVarRef.getVariable().getName());
                    inits.pr(
                            PythonPortGenerator.generatePythonPortVariableInReaction(
                                    triggerAsVarRef));
                }
        ...
        for (VarRef src : ASTUtils.convertToEmptyListIfNull(reaction.getSources())) {
            if (src.getVariable() instanceof Output) {
                ...
                generatedParams.add(
                        src.getContainer().getName() + "_" + src.getVariable().getName());
        ...
        for (VarRef effect : ASTUtils.convertToEmptyListIfNull(reaction.getEffects())) {
            if (effect.getVariable() instanceof Input) {
                generatedParams.add(
                        effect.getContainer().getName() + "_" + effect.getVariable().getName());
    ...
    private static String generateVariableToSendPythonReaction(
            VarRef varRef,
            Set<Action> actionsAsTriggers,
            ReactorDecl decl,
            List<String> pyObjects) {
        if (varRef.getVariable() instanceof Port) {
            return PythonPortGenerator.generatePortVariablesToSendToPythonReaction(
                    pyObjects, varRef, decl);
        } else if (varRef.getVariable() instanceof Action) {
            actionsAsTriggers.add((Action) varRef.getVariable());
            PythonPortGenerator.generateActionVariableToSendToPythonReaction(
                    pyObjects, (Action) varRef.getVariable(), decl);
        }
        return "";
    }
    ...
    public static String generateCPythonReactionLinkers(
            ReactorInstance instance, Instantiation mainDef) {
    ...
    public static String generateCPythonReactionLinker(
            ReactorInstance instance, ReactionInstance reaction, String nameOfSelfStruct) {
        CodeBuilder code = new CodeBuilder();
        code.pr(
    ...
    /**
     * Generate code to link "pythonFunctionName" to "cpythonFunctionName" in "nameOfSelfStruct" of
     * "instance".
     *
     ...
     */
    private static String generateCPythonFunctionLinker(
        ...
        return String.join(
                ...
                "    lf_print_error_and_exit(\"Could not load function "
                        + pythonFunctionName
                        + "\");",
                "}");
    }
     ...
     *     generated code)
     * @param reactionParameters The parameters to the deadline violation function, which are the
     *     same as the reaction function
     */
    ...
    public static String generatePythonReaction(
        ...
        CodeBuilder code = new CodeBuilder();
        List<String> reactionParameters =
                new ArrayList<>(); // Will contain parameters for the function (e.g., Foo(x,y,z,...)
        CodeBuilder inits =
                new CodeBuilder(); // Will contain initialization code for some parameters
        PythonReactionGenerator.generatePythonReactionParametersAndInitializations(
                reactionParameters, inits, reactor, reaction);
        code.pr(
    ...
    /**
     * Return the function name of the STP violation handler function inside the self struct in the
     * .c file.
     *
     * @param reactionIndex The reaction index.
     * @return The function name for the reaction.
