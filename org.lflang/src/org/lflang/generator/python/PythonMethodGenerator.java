...
public class PythonMethodGenerator {
    ...
    public static String generateMethods(Reactor reactor) {
        return ASTUtils.allMethods(reactor).stream().map(m -> generateMethod(m)).collect(Collectors.joining());
    }
    ...
    private static String generateMethodArgumentList(Method method) {
        return String.join(
                ", ",
                method.getArguments().stream().map(MethodArgument::getName).collect(Collectors.toList()));
    }
}
