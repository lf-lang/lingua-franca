...
public class PythonValidator extends org.lflang.generator.Validator {
    ...
    public PythonValidator(
            FileConfig fileConfig, ErrorReporter errorReporter, Map<Path, CodeMap> codeMaps, Set<String> protoNames) {
                ...
                new ValidationStrategy() {
                    ...
                    private boolean tryReportTypical(String[] lines, int i) {
                        ...
                        if (main.matches()) {
                            ...
                            CodeMap map = codeMaps.get(fileConfig
                                    .getSrcGenPkgPath()
                            ...
                            if (map == null) {
                                ...
                                for (Path lfFile : map.lfSourcePaths()) {
                                    ...
                                    // TODO: We could be more precise than just getting the right line, but the way the
                                    // output
                ...
                new ValidationStrategy() {
                    ...
                    private boolean shouldIgnore(PylintMessage message) {
                        ...
                        if (message.symbol.equals("trailing-whitespace") || message.symbol.equals("line-too-long"))
                            return true;
                        // This filters out Pylint messages concerning missing members in types defined by protocol
                        // buffers.
                    ...
                    private void bestEffortReport(
                        ...
                        if (!lfEnd.equals(Position.ORIGIN) && !lfStart.equals(Position.ORIGIN)) { // Ideal case
                            ...
                            if (lfStart.equals(Position.ORIGIN))
                                lfStart = adjust.apply(
                                        Position.fromZeroBased(lfStart.getZeroBasedLine(), Integer.MAX_VALUE));
                            // FIXME: It might be better to improve style of generated code instead of quietly returning
                            // here.
