...
public class PythonValidator extends org.lflang.generator.Validator {
    ...
    /**
     * The pattern typically followed by the message that typically follows the main diagnostic
     * line.
     */
    ...
    private static final class PylintMessage {
        ...
        public Position getStart() {
            ...
            // Use 0 as fallback for the column. This will cause bugs by taking some positions out
            // of the line
            // adjuster's range.
            if (line != null) return Position.fromZeroBased(line - 1, 0);
            // This fallback will always fail with the line adjuster, but at least the program will
            // not crash.
        ...
        return List.of(
                new ValidationStrategy() {
                    ...
                    public LFCommand getCommand(Path generatedFile) {
                        return LFCommand.get(
                                ...
                                List.of(
                                        "-c",
                                        "import compileall; compileall.compile_dir('.', quiet=1)"),
                                true,
                     ...
                     * @param i The current index at which a message may start. Guaranteed to be
                     *     less than {@code lines.length - 3}.
                    ...
                    private boolean tryReportTypical(String[] lines, int i) {
                        ...
                        String message =
                                messageMatcher.matches() ? messageMatcher.group() : "Syntax Error";
                        if (main.matches()) {
                            ...
                            Position genPosition =
                                    Position.fromOneBased(
                                            line,
                                            Integer.MAX_VALUE); // Column is just a placeholder.
                            if (map == null) {
                                errorReporter.report(
                                        null,
                                        DiagnosticSeverity.Error,
                                        message,
                                        1); // Undesirable fallback
                            } else {
                                for (Path lfFile : map.lfSourcePaths()) {
                                    Position lfPosition = map.adjusted(lfFile, genPosition);
                                    // TODO: We could be more precise than just getting the right
                                    // line, but the way the output
                                    //  is formatted (with leading whitespace possibly trimmed) does
                                    // not make it easy.
                                    errorReporter.report(
                                            lfFile,
                                            DiagnosticSeverity.Error,
                                            message,
                                            lfPosition.getOneBasedLine());
                                }
                            }
                            return true;
                        }
                        ...
                        if (main.matches()) {
                            ...
                            Iterable<CodeMap> relevantMaps =
                                    codeMaps.keySet().stream()
                                            ...
                                            ::iterator;
                            for (CodeMap map :
                                    relevantMaps) { // There should almost always be exactly one of
                                // these
                                for (Path lfFile : map.lfSourcePaths()) {
                                    errorReporter.report(
                                            lfFile,
                                            ...
                                            map.adjusted(lfFile, Position.fromOneBased(line, 1))
                                                    .getOneBasedLine());
                                }
                            }
                        }
                ...
                new ValidationStrategy() {
                        ...
                        return (validationOutput, errorReporter, codeMaps) -> {
                            ...
                            try {
                                ...
                                errorReporter.reportWarning(
                                        "Failed to parse linter output. The Lingua Franca code"
                                                + " generator is tested with Pylint version 2.12.2."
                                                + " Consider updating Pylint if you have an older"
                                                + " version.");
                            }
                        };
                    }
                    ...
                    private boolean shouldIgnore(PylintMessage message) {
                        // Code generation does not preserve whitespace, so this check is
                        // unreliable.
                        if (message.symbol.equals("trailing-whitespace")
                                || message.symbol.equals("line-too-long")) return true;
                        // This filters out Pylint messages concerning missing members in types
                        // defined by protocol buffers.
                    ...
                    private void bestEffortReport(
                            ...
                            String humanMessage) {
                        if (!lfEnd.equals(Position.ORIGIN)
                                && !lfStart.equals(Position.ORIGIN)) { // Ideal case
                            errorReporter.report(file, severity, humanMessage, lfStart, lfEnd);
                        } else { // Fallback: Try to report on the correct line, or failing that,
                            // just line 1.
                            if (lfStart.equals(Position.ORIGIN))
                                lfStart =
                                        adjust.apply(
                                                Position.fromZeroBased(
                                                        lfStart.getZeroBasedLine(),
                                                        Integer.MAX_VALUE));
                            // FIXME: It might be better to improve style of generated code instead
                            // of quietly returning here.
                            if (lfStart.equals(Position.ORIGIN)
                                    && severity != DiagnosticSeverity.Error) return;
                            errorReporter.report(
                                    file, severity, humanMessage, lfStart.getOneBasedLine());
                        }
                    }
