...
public class PythonReactorGenerator {
    /**
     * Wrapper function for the more elaborate generatePythonReactorClass that keeps track of
     * visited reactors to avoid duplicate generation
     *
    ...
    public static String generatePythonClass(
        ...
        if (!instantiatedClasses.contains(className)) {
            ...
            pythonClasses.pr(
                    PythonPreambleGenerator.generatePythonPreambles(reactor.getPreambles()));
            ...
            pythonClasses.pr(
                    PythonReactionGenerator.generatePythonReactions(reactor, reactionToGenerate));
    ...
    /**
     * Generate code that instantiates and initializes parameters and state variables for a reactor
     * 'decl'.
     *
    ...
    /**
     * Instantiate classes in Python, as well as subclasses. Instances are always instantiated as a
     * list of className = [_className, _className, ...] depending on the size of the bank. If there
     * is no bank or the size is 1, the instance would be generated as className = [_className]
     *
     * @param instance The reactor instance to be instantiated
     * @param main The main reactor
     */
    public static String generatePythonClassInstantiations(
            ReactorInstance instance, ReactorInstance main) {
        CodeBuilder code = new CodeBuilder();
        ...
        if (instance.getWidth() > 0) {
            ...
            // Non-bank reactor instances will be a list of size 1.         var reactorClass =
     ...
     */
    private static String generatePythonClassInstantiation(
            ReactorInstance instance, String className) {
        CodeBuilder code = new CodeBuilder();
