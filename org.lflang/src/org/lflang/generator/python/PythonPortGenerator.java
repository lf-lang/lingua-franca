...
public class PythonPortGenerator {
    ...
    /**
     * Generate code to convert C actions to Python action capsules. See pythontarget.h for details.
     *
     * @param pyObjects A string containing a list of comma-separated expressions that will create
     *     the action capsules.
    ...
    public static void generateActionVariableToSendToPythonReaction(
        ...
        // However, sometimes token might not be initialized. Therefore, this function has an
        // internal check for NULL in case token is not initialized.
     ...
     *
     * <p>The port may be an input of the reactor or an output of a contained reactor.
     *
     * @param pyObjects A string containing a list of comma-separated expressions that will create
     *     the port capsules.
    ...
    public static String generatePortVariablesToSendToPythonReaction(
        ...
        if (port.getVariable() instanceof Input) {
            generateInputVariablesToSendToPythonReaction(
                    pyObjects, (Input) port.getVariable(), decl);
            return "";
        } else {
    ...
    /**
     * Generate into the specified string builder the code to send local variables for output ports
     * to a Python reaction function from the "self" struct.
     *
     * @param output The output port.
     */
    public static void generateOutputVariablesToSendToPythonReaction(
            List<String> pyObjects, Output output) {
    ...
    /**
     * Generate into the specified string builder the code to send local variables for input ports
     * to a Python reaction function from the "self" struct.
     *
    ...
    /**
     * Generate into the specified string builder the code to pass local variables for sending data
     * to an input of a contained reaction (e.g. for a deadline violation).
     *
    ...
    public static String generateVariablesForSendingToContainedReactors(
        ...
        if (definition.getWidthSpec() != null) {
            ...
            if (ASTUtils.isMultiport(port)) {
                widthSpec =
                        "self->_lf_"
                                + definition.getName()
                                + "[i]."
                                + generateWidthVariable(port.getName());
            }
            ...
            if (ASTUtils.isMultiport(port)) {
                pyObjects.add(
                        generateConvertCPortToPy(definition.getName() + "." + port.getName()));
            } else {
                pyObjects.add(
                        generateConvertCPortToPy(
                                definition.getName() + "." + port.getName(),
    ...
    public static String generatePythonListForContainedBank(
            String reactorName, Port port, String widthSpec) {
        return String.join(
                "\n",
                "PyObject* "
                        + reactorName
                        + "_py_list = PyList_New("
                        + generateWidthVariable(reactorName)
                        + ");",
                "if(" + reactorName + "_py_list == NULL) {",
                "    lf_print_error(\"Could not create the list needed for "
                        + reactorName
                        + ".\");",
                "    if (PyErr_Occurred()) {",
                "        PyErr_PrintEx(0);",
                ...
                "            "
                        + generateConvertCPortToPy(
                                reactorName + "[i]." + port.getName(), widthSpec),
                "        ) != 0) {",
                "        lf_print_error(\"Could not add elements to the list for "
                        + reactorName
                        + ".\");",
                "        if (PyErr_Occurred()) {",
                "            PyErr_PrintEx(0);",
                "            PyErr_Clear(); // this will reset the error indicator so we can run"
                        + " Python code again",
                "        }",
                "        /* Release the thread. No Python API allowed beyond this point. */",
                "        PyGILState_Release(gstate);",
    ...
    public static String generateAliasTypeDef(
            Reactor r, Port port, boolean isTokenType, String genericPortType) {
        return "typedef "
                + genericPortType
                + " "
                + CGenerator.variableStructType(port, r, false)
                + ";";
    }
    ...
    public static String generatePythonPortVariableInReaction(VarRef port) {
        ...
        String tryStatement =
                "try: " + containerName + "  # pylint: disable=used-before-assignment";
