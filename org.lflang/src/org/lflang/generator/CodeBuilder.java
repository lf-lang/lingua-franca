...
public class CodeBuilder {
     ...
     * output. Use this when multiple lines of output code are all due to the same source line in
     * the .lf file.
     *
     ...
     * for loop that iterates over the bank members using a standard index variable whose name is
     * that returned by {@link CUtil#bankIndex(ReactorInstance)}. If the reactor is null or is not a
     * bank, then this simply starts a scoped block by printing an opening curly brace. This also
     * adds a declaration of a pointer to the self struct of the reactor or bank member.
     *
     * <p>This block is intended to be nested, where each block is put within a similar block for
     * the reactor's parent. This ensures that all (possibly nested) bank index variables are
    ...
    /**
     * If the specified port is a multiport, then start a specified iteration over the channels of
     * the multiport using as the channel index the variable name returned by {@link
     * CUtil#channelIndex(PortInstance)}. If the port is not a multiport, do nothing. This is
     * required to be followed by {@link #endChannelIteration(PortInstance)}.
     *
     * @param port The port.
     */
     ...
     *     the string returned by {@link CUtil#channelIndexName(PortInstance)}.
        ...
        if (range.width > 1) {
            pr(
                    String.join(
                            "\n",
                            "int range_start[] =  { "
                                    + joinObjects(rangeMR.getDigits(), ", ")
                                    + " };",
                            "int range_radixes[] = { "
                                    + joinObjects(rangeMR.getRadixes(), ", ")
                                    + " };",
                            "int permutation[] = { "
                                    + joinObjects(range.permutation(), ", ")
                                    + " };",
    ...
    /**
     * Start a scoped block that iterates over the specified pair of ranges. The destination range
     * can be wider than the source range, in which case the source range is reused until the
     * destination range is filled. The following integer variables will be defined within the
     * scoped block:
     *
     * <p>* src_channel: The channel index for the source. * src_bank: The bank index of the source
     * port's parent. * src_runtime: The runtime index of the source port's parent or the parent's
     * parent (if the source is an input).
     *
     * <p>* dst_channel: The channel index for the destination. * dst_bank: The bank index of the
     * destination port's parent. * dst_runtime: The runtime index of the destination port's parent
     * or the parent's parent (if destination is an output).
     *
     * <p>For convenience, the above variable names are defined in the private class variables sc,
     * sb, sr, and dc, db, dr.
     *
     * <p>This block should NOT be nested, where each block is put within a similar block for the
     * reactor's parent. Within the created block, every use of {@link
     ...
     * variable names.
     *
     * <p>This must be followed by a call to {@link #endScopedRangeBlock(SendRange, RuntimeRange)}.
     *
            ...
            pr(
                    String.join(
                            ...
                            "int src_value[] =  { "
                                    ...
                                    + " }; // Will be incremented.",
                            "int src_radixes[] = { "
                                    + joinObjects(srcRangeMR.getRadixes(), ", ")
                                    + " };",
                            "int src_permutation[] = { "
                                    + joinObjects(srcRange.permutation(), ", ")
                                    + " };",
