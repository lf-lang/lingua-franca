...
public abstract class MalleableString {
    ...
    private static final class Sequence extends MalleableString {
        ...
        @Override
        public RenderResult render(int indentation, String singleLineCommentPrefix, boolean codeMapTag, EObject enclosingEObject) {
            List<RenderResult> componentRenderings = components.stream()
                .map(malleableString ->
                    // The code map tags *should* not affect the correctness of the formatting since most
        ...
        public void findBestRepresentation(
            ...
            if (
                components.stream().noneMatch(
                    it -> it.render(indentation, singleLineCommentPrefix, false, null)
    ...
    private static final class Fork extends MalleableStringWithAlternatives<MalleableString> {
        ...
        @Override
        public RenderResult render(int indentation, String singleLineCommentPrefix, boolean codeMapTag, EObject enclosingEObject) {
            return getChosenPossibility().render(
                indentation,
                singleLineCommentPrefix,
                codeMapTag,
                sourceEObject != null ? sourceEObject : enclosingEObject
