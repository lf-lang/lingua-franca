...
public class IsEqual extends LfSwitch<Boolean> {
    ...
    public Boolean caseVariable(Variable object) {
        throw thereIsAMoreSpecificCase(
                Variable.class, TypedVariable.class, Timer.class, Mode.class);
    }
    ...
    public Boolean caseVarRef(VarRef object) {
        return new ComparisonMachine<>(object, VarRef.class)
                .equalAsObjects(
                        varRef ->
                                varRef.getVariable() instanceof Mode
                                        ? null
                                        : varRef.getVariable().getName())
                .equivalent(
                        varRef ->
                                varRef.getVariable() instanceof Mode ? null : varRef.getVariable())
                .equalAsObjects(
                        varRef ->
                                varRef.getContainer() == null
                                        ? null
    ...
    public Boolean caseCodeExpr(CodeExpr object) {
        return new ComparisonMachine<>(object, CodeExpr.class)
                .equivalent(CodeExpr::getCode)
                .conclusion;
    }
    ...
    private class ComparisonMachine<E extends EObject> {
         ...
         * projectionToClassRepresentatives} maps each object to some semantically equivalent
         * object.
         */
        <T> ComparisonMachine<E> equalAsObjectsModulo(
            ...
            if (propertyGetter.apply(object) instanceof EObject) {
                throw new IllegalArgumentException(
                        "EObjects should be compared for semantic equivalence, not object"
                                + " equality.");
            }
            propertyGetter = projectionToClassRepresentatives.compose(propertyGetter);
            if (conclusion)
                conclusion =
                        Objects.equals(propertyGetter.apply(object), propertyGetter.apply(other));
            return this;
        }
        ...
        /**
         * Conclude false if the two properties are not semantically equivalent parse nodes, given
         * that {@code projectionToClassRepresentatives} maps each parse node to some semantically
         * equivalent node.
         */
        <T extends EObject> ComparisonMachine<E> equivalentModulo(
            ...
            if (conclusion)
                conclusion =
                        new IsEqual(propertyGetter.apply(object))
