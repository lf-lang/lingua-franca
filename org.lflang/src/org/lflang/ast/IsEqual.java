...
public class IsEqual extends LfSwitch<Boolean> {
    ...
    public Boolean caseReactorDecl(ReactorDecl object) {
        return new ComparisonMachine<>(object, ReactorDecl.class).equalAsObjects(ReactorDecl::getName).conclusion;
    }
    ...
    public Boolean caseSerializer(Serializer object) {
        return new ComparisonMachine<>(object, Serializer.class).equalAsObjects(Serializer::getType).conclusion;
    }

    @Override
    public Boolean caseKeyValuePairs(KeyValuePairs object) {
        return new ComparisonMachine<>(object, KeyValuePairs.class).listsEquivalent(KeyValuePairs::getPairs).conclusion;
    }
    ...
    public Boolean caseArray(Array object) {
        return new ComparisonMachine<>(object, Array.class).listsEquivalent(Array::getElements).conclusion;
    }
    ...
    public Boolean caseVarRef(VarRef object) {
        return new ComparisonMachine<>(object, VarRef.class)
                .equalAsObjects(varRef -> varRef.getVariable() instanceof Mode
                        ? null
                        : varRef.getVariable().getName())
                .equivalent(varRef -> varRef.getVariable() instanceof Mode ? null : varRef.getVariable())
                .equalAsObjects(varRef -> varRef.getContainer() == null
                        ? null
                        : varRef.getContainer().getName())
    ...
    public Boolean caseCode(Code object) {
        return new ComparisonMachine<>(object, Code.class)
                .equalAsObjectsModulo(
                        Code::getBody, s -> s == null ? null : s.strip().stripIndent())
                .conclusion;
    }

    @Override
    public Boolean caseLiteral(Literal object) {
        return new ComparisonMachine<>(object, Literal.class).equalAsObjects(Literal::getLiteral).conclusion;
    }
    ...
    private UnsupportedOperationException thereIsAMoreSpecificCase(
        ...
        return new UnsupportedOperationException(String.format(
                "%ss are %s, so the methods " + "corresponding to those types should be invoked instead.",
                thisCase.getName(),
    ...
    private class ComparisonMachine<E extends EObject> {
        ...
        <T extends EObject> ComparisonMachine<E> equivalentModulo(
            ...
            propertyGetter = projectionToClassRepresentatives.compose(propertyGetter);
            if (conclusion)
                conclusion = new IsEqual(propertyGetter.apply(object)).doSwitch(propertyGetter.apply(other));
            return this;
        }
    }
}
