...
class LspTests extends LfInjectedTestBase {
                  ...
                  diagnostics ->
                      alteredTest.getBadLines().stream()
                          .allMatch(
                              badLine -> {
                                System.out.print(
                                    "Expecting an error to be reported at line " + badLine + "...");
                                boolean result =
                                    NOT_SUPPORTED.test(diagnostics)
                                        || diagnostics.stream()
                                            .anyMatch(
                                                diagnostic ->
  ...
  private Iterable<? extends TestCategory> selectedCategories() {
    return () ->
        Arrays.stream(TestCategory.values())
            .filter(category -> Arrays.stream(EXCLUDED_CATEGORIES).noneMatch(category::equals))
            .iterator();
  }
  ...
  private static Predicate<List<Diagnostic>> diagnosticsIncludeText(
    ...
    return diagnostics ->
        diagnostics.stream().anyMatch(d -> d.getMessage().toLowerCase().contains(requiredText));
  }
