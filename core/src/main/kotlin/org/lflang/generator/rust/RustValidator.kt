...
class RustValidator(
    ...
    val metadata: RustMetadata? by lazy {
        val command =
            LFCommand.get("cargo", listOf("metadata", "--format-version", "1"), true, fileConfig.srcGenPkgPath)
                ?: return@lazy null
    ...
    private val rustValidationStrategy = object : ValidationStrategy {
        ...
        override fun getOutputReportingStrategy() = DiagnosticReporting.Strategy { validationOutput, errorReporter, map ->
            for (messageLine in validationOutput.lines()) {
                if (messageLine.isBlank()
                    || mapper.readValue(messageLine, RustOutput::class.java).reason != COMPILER_MESSAGE_REASON
                ) continue
    ...
    override fun getPossibleStrategies(): Collection<ValidationStrategy> = listOf(rustValidationStrategy)

    override fun getBuildReportingStrategies(): Pair<DiagnosticReporting.Strategy, DiagnosticReporting.Strategy> = Pair(
        rustValidationStrategy.errorReportingStrategy,
        rustValidationStrategy.outputReportingStrategy
    )
}
