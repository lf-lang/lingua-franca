 ...
 * 
 * An attribute is an annotation using the `@` syntax.
 * For example, `@label("My Label")` is an attribute.
 * It is associated with whatever node comes immediately after it.
 *
 * @author Shaokai Lin
...
public class AttributeUtils {
  ...
  /**
   * Return the attributes declared on the given node or null if the node does not support
  ...
  /**
   * Return the first argument specified for the attribute or null if the attribute is not
  ...
  /**
   * For an attribute with the given name on the given AST node, return a map of the
   * attribute parameters to their values.
   * This should only be used on attributes that are expected to have a single argument. This
  ...
  /**
   * Retrieve a specific annotation in a comment associated with the given model element in
   * the AST.
   * This will look for a comment. If one is found, it searches for the given annotation
   ...
   * Return null if no such parameter is found.
  ...
  /**
   * Return the parameter of the given attribute with the given name and interpret it as a
   * boolean.
   ...
   * Currently, this is only used for synthesized reactions in the context of federated
   * execution in Python.
  ...
  /**
   * Return the declared icon of the node, as given by the @icon annotation, or null if there
  ...
  /**
   * Return the {@code @side} annotation for the given node (presumably a port) or null if
