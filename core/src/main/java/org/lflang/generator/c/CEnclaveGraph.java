...
public class CEnclaveGraph {
      ...
      for (PortInstance input : enclave.inputs) {
        if (input.eventualSources().size() == 1) {
          ...
          if (input.getDependentPorts().size() == 1) {
            ...
            // FIXME: Can eventual source be more than 1 for an input port? It is an error condition
            // for us, at least
          }
  ...
  private boolean hasZeroDelayCycle(
    ...
    for (EnclaveConnection edge :
        graph.get(current).stream().filter(e -> e.getDelay().toNanoSeconds() == 0).toList()) {
  ...
  public void addEdge(
      ReactorInstance source, ReactorInstance target, TimeValue delay, boolean isPhysical) {
