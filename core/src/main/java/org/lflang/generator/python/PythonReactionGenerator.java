...
public class PythonReactionGenerator {
  ...
  public static String generateCPythonDeadlineCaller(
      TypeParameterizedReactor r, int reactionIndex, List<String> pyObjects) {
  ...
  private static String generateCPythonFunctionCaller(
    ...
    code.pr(
        String.join(
            ...
            "    if (PyErr_Occurred()) {",
            "        PyErr_PrintEx(0);",
            "        PyErr_Clear(); // this will reset the error indicator so we can run Python"
                + " code again",
            "    }",
  ...
  /**
   * Return the function name of the STP violation handler function inside the self struct in the .c
   * file.
   *
   * @param reactionIndex The reaction index.
   * @return The function name for the reaction.
