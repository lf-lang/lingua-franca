...
public class PythonPortGenerator {
  ...
  public static void generateActionVariableToSendToPythonReaction(
    ...
    // However, sometimes token might not be initialized. Therefore, this function has an internal
    // check for NULL in case token is not initialized.
  ...
  public static String generateVariablesForSendingToContainedReactors(
    ...
    if (definition.getWidthSpec() != null) {
      ...
      if (ASTUtils.isMultiport(port)) {
        widthSpec =
            "self->_lf_" + definition.getName() + "[i]." + generateWidthVariable(port.getName());
      }
      ...
      if (ASTUtils.isMultiport(port)) {
        ...
        pyObjects.add(
            generateConvertCPortToPy(
                definition.getName() + "." + port.getName(), NONMULTIPORT_WIDTHSPEC));
      }
    }
  ...
  public static String generatePythonListForContainedBank(
    ...
    return String.join(
        ...
        "    if (PyErr_Occurred()) {",
        "        PyErr_PrintEx(0);",
        "        PyErr_Clear(); // this will reset the error indicator so we can run Python code"
            + " again",
        "    }",
