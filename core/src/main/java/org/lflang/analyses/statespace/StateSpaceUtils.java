...
public class StateSpaceUtils {
  ...
  public static StateSpaceDiagram generateStateSpaceDiagram(
      StateSpaceExplorer explorer, StateSpaceExplorer.Phase explorePhase, ReactorInstance main, Tag horizon, TargetConfig targetConfig, Path graphDir, String graphPrefix) {
    // Get a list of initial events according to the exploration phase.
    List<Event> initialEvents = StateSpaceExplorer.addInitialEvents(main, explorePhase, targetConfig);
    
    // Explore the state space with the phase specified.
    StateSpaceDiagram stateSpaceDiagram = explorer.explore(main, horizon, explorePhase, initialEvents);
  ...
  public static List<StateSpaceDiagram> generateAsyncStateSpaceDiagrams(
    ...
    if (explorePhase != Phase.ASYNC)
        throw new RuntimeException("The exploration mode must be ASYNC inside generateStateSpaceDiagramAsync().");
    ...
    for (int i = 0; i < asyncEvents.size(); i++) {
      ...
      // Explore the state space with the phase specified.
      StateSpaceDiagram stateSpaceDiagram = explorer.explore(main, new Tag(0, 0, true), explorePhase, Arrays.asList(event));
   ...
   * 
   * FIXME: This is not an efficient algorithm since every time a new diagram
   * gets merged, the size of the merged diagram could blow up exponentially. A
   * one-pass algorithm would be better.
   * 
  ...
  public static StateSpaceDiagram mergeAsyncDiagramsIntoDiagram(
    ...
    StateSpaceDiagram targetDiagram
  ) {
  ...
  /**
   * Merge an async diagram into a non-async fragment based on minimum
   * spacing, which in this case is interpreted as the period at which the
   * presence of the physical action.
   * 
   * In the state space exploration, generate a diagram for EACH physical
   ...
   * ASSUMPTIONS:
   * 
   * 1. min spacing <= hyperperiod. If minimum space > hyperperiod, we then
   *    need to unroll the synchronous diagram.
   * 
   * 2. min spacing is a divisor of the hyperperiod. This simplifies the
   *    placement of async nodes and removes the need to account for the shift
   *    of async node sequence over multiple iterations. To relax this
   *    assumption, we need to recalculate the hyperperiod of a periodic diagram
   *    with the addition of async nodes. 
   * 
   * 3. Physical action does not occur at the same tag as synchronous nodes.
   *    This removes the need to merge two nodes at the same tag. This is not
   *    necessarily hard, however.
   * 
   * 4. The sequence of async nodes is shifted 1 nsec after the sync sequence
   *    starts. This is a best effort approach to avoid collision between sync
   *    and async nodes, in which case assumption 3 is activated. This also
   *    makes it easy for the merge algorithm to work on a single diagram
   *    without worrying about the temporal relations between its async nodes
   *    and other diagrams' async nodes, i.e., enabling a compositional merge
   *    strategy. 
   * 
   * 5. Sometimes, the actual minimum spacing in the schedule could be greater
   *    than the specified minimum spacing.
   *    This could occur due to a few reasons:
   *    A) Assumption 3;
   *    B) In the transition between the initialization phase and the periodic
   *    phase, the LAST async node in the initialization phase can be dropped to
   *    make sure the FIRST async node in the periodic phase does not break the
   *    min spacing requirement due to compositional merge strategy. This is not
   *    a problem if we consider a global merge strategy, i.e., using a single
   *    diagram to represent the logical behavior and merge the async nodes
   *    across multiple phases at once.
   */
  public static StateSpaceDiagram mergeAsyncDiagramIntoDiagram(
    StateSpaceDiagram asyncDiagram,
    StateSpaceDiagram targetDiagram
  ) {
    ...
    while (!stop) {
      ...
      if (asyncTag.compareTo(current.getTag()) < 0) {
        ...
        // Update async tag
        asyncTag = new Tag(asyncTag.timestamp + asyncDiagram.getMinSpacing().toNanoSeconds(), 0, false);
        ...
        if (current == targetDiagram.loopNode) 
          mergedDiagram.loopNode = current;
