...
The \texttt{Ramp} reactor sends messages that are a counting sequence beginning at 0 at regular time intervals
with period given by its parameter \texttt{p}.
It declares an output named \texttt{out} of type \texttt{int} and a
\keyword{clock}, which will trigger reactions periodically.
The \keyword{constructor} declares and initializes a state variable \texttt{count}.
This reactor has one \keyword{reaction}, which is triggered by the \keyword{clock} named \texttt{c} and produces
an output on the port named \texttt{out}.
...
\todo{Examples don't illustrate ordering of reactions.}
