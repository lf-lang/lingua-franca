...
All messages communicated between actors in our model are timestamped. Message exchange is coordinated under a discrete-event (DE) semantics, explained in more detail in \cite{LeeEtAl:7:DiscreteEvents}, guaranteeing that (1) actors observe produced messages in timestamp order, and (2) actors cannot produce messages with a timestamp $t$ until all anti-dependent~\ag{what does this mean?} inputs with timestamp $t$ are known. 
