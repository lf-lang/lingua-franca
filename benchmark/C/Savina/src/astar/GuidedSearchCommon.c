#include "GuidedSearchCommon.h"
#include "GridNode.h"
#include <math.h>
#include <stdlib.h>

//Need to find the C equivalent of this:
...
//defaults, but actual values specified in command line parameters
...
int calcPriority(GridNode * gridNode) {
	...
	int distDiff = max(GRID_SIZE - gridNode->i, max(GRID_SIZE - gridNode->j, GRID_SIZE - gridNode->k));
	int priorityUnit = distDiff / PRIORITY_GRANULARITY;
	int resultPriority = fabs(availablePriorities - priorityUnit);
	return resultPriority;
}

void WorkMessage(GridNode * node, GridNode * target) {
    this->priority = calcPriority(node)
}

void initializeData() {
    //probably need a new check here
    if(allNodes == nullptr) {
			...
			for (int i = 0; i < 2; i++) {
				for (int j = 0; j < 2; j++) {
					for (int k = 0; k < 2; k++) {
						...
						if(addNeighbor) {
							const int newI = min(GRID_SIZE - 1, gridNode->i + i);
							const int newJ = min(GRID_SIZE - 1, gridNode->j + j);
							const int newK = min(GRID_SIZE - 1, gridNode->k + k);
