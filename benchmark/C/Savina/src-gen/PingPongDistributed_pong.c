...
typedef struct {
    ...
    reaction_t* ___networkMessage_reactions[1];
    trigger_t ___networkMessage_0;
} pingpong_self_t;
void pingpongreaction_function_1(void* instance_args) {
    struct pong{
        pong_receive_t* receive;
    } pong;
    ...
    // Set the fields of the action struct to match the current trigger.
    networkMessage->is_present = self->___networkMessage.is_present;
    networkMessage->has_value = ((self->___networkMessage.token) != NULL && (self->___networkMessage.token)->value != NULL);
    networkMessage->token = (self->___networkMessage.token);
    if (networkMessage->has_value) {
        networkMessage->value = *(int*)(self->___networkMessage.token)->value;
    }
    pong.receive = &(self->__pong.receive);
    // Receiving from ping.send in federate ping to pong.receive in federate pong
    SET(pong.receive, networkMessage->value);
}
void pingpongreaction_function_2(void* instance_args) {
    struct pong{
        pong_send_t* send;
    } pong;
    pingpong_self_t* self = (pingpong_self_t*)instance_args;
    pong.send = self->__pong.send;
    pingpong_networkMessage_0_t* networkMessage_0 = &self->__networkMessage_0;
    // Sending from pong.send in federate pong to ping.receive in federate ping
    size_t message_length = sizeof(int);
    send_message(_lf_federate_sockets_for_outbound_p2p_connections[0], P2P_MESSAGE, 0, 0, message_length, (unsigned char*)&pong.send->value);
}
pingpong_self_t* new_PingPong() {
    ...
    #line 41 "file:C:\\Users\\soyer\\git\\lingua-franca\\benchmark\\C\\Savina\\PingPongDistributed.lf"
    self->__pong.send_reactions[0] = &self->___reaction_2;
    #line 41 "file:C:\\Users\\soyer\\git\\lingua-franca\\benchmark\\C\\Savina\\PingPongDistributed.lf"
    self->__pong.send_trigger.reactions = self->__pong.send_reactions;
    #line 41 "file:C:\\Users\\soyer\\git\\lingua-franca\\benchmark\\C\\Savina\\PingPongDistributed.lf"
    self->__pong.send_trigger.last = NULL;
    #line 41 "file:C:\\Users\\soyer\\git\\lingua-franca\\benchmark\\C\\Savina\\PingPongDistributed.lf"
    self->__pong.send_trigger.number_of_reactions = 1;
    self->___reaction_1.number = 1;
    self->___reaction_1.function = pingpongreaction_function_1;
    self->___reaction_1.self = self;
    self->___reaction_1.deadline_violation_handler = NULL;
    self->___reaction_1.tardy_handler = NULL;
    self->___reaction_2.number = 2;
    self->___reaction_2.function = pingpongreaction_function_2;
    ...
    self->___networkMessage_reactions[0] = &self->___reaction_1;
    self->___networkMessage.reactions = &self->___networkMessage_reactions[0];
    self->___networkMessage.number_of_reactions = 1;
...
void __initialize_trigger_objects() {
    ...
    __action_table[0] = &pingpong_self->___networkMessage;
    // Populate arrays of trigger pointers.
    pingpong_self->bank_index = 0;
    // Total number of outputs produced by the reaction.
    pingpong_self->___reaction_1.num_outputs = 1;
    // Allocate arrays for triggering downstream reactions.
    if (pingpong_self->___reaction_1.num_outputs > 0) {
        pingpong_self->___reaction_1.output_produced = (bool**)malloc(sizeof(bool*) * pingpong_self->___reaction_1.num_outputs);
        pingpong_self->___reaction_1.triggers = (trigger_t***)malloc(sizeof(trigger_t**) * pingpong_self->___reaction_1.num_outputs);
        pingpong_self->___reaction_1.triggered_sizes = (int*)malloc(sizeof(int) * pingpong_self->___reaction_1.num_outputs);
    }
    // Initialize the output_produced array.
    pingpong_self->___reaction_1.output_produced[0]
    = &pingpong_self->__pong.receive.is_present
    ;
    // Total number of outputs produced by the reaction.
    pingpong_self->___reaction_2.num_outputs = 0;
    // Allocate arrays for triggering downstream reactions.
    if (pingpong_self->___reaction_2.num_outputs > 0) {
        pingpong_self->___reaction_2.output_produced = (bool**)malloc(sizeof(bool*) * pingpong_self->___reaction_2.num_outputs);
        pingpong_self->___reaction_2.triggers = (trigger_t***)malloc(sizeof(trigger_t**) * pingpong_self->___reaction_2.num_outputs);
        pingpong_self->___reaction_2.triggered_sizes = (int*)malloc(sizeof(int) * pingpong_self->___reaction_2.num_outputs);
    }
    // Initialize the output_produced array.
    // Reaction 1 of PingPong does not depend on one maximal upstream reaction.
    pingpong_self->___reaction_1.last_enabling_reaction = NULL;
    // Reaction 1 of PingPong triggers 1 downstream reactions through port PingPong.pong.receive.
    pingpong_self->___reaction_1.triggered_sizes[0] = 1;
    // For reaction 1 of PingPong, allocate an
    // array of trigger pointers for downstream reactions through port PingPong.pong.receive
    trigger_t** pingpong_1_0 = (trigger_t**)malloc(1 * sizeof(trigger_t*));
    pingpong_self->___reaction_1.triggers[0] = pingpong_1_0;
    // Point to destination port PingPong.pong.receive's trigger struct.
    pingpong_1_0[0] = &pingpong_pong_self->___receive;
    // Reaction 2 of PingPong depends on one maximal upstream reaction.
    pingpong_self->___reaction_2.last_enabling_reaction = &(pingpong_pong_self->___reaction_0);
    pingpong_self->__pong.receive.
    num_destinations = 1;
    ...
    pingpong_pong_self->___reaction_0.last_enabling_reaction = &(pingpong_self->___reaction_1);
    ...
    // Port PingPong.pong.send has reactions in its parent's parent.
    // Point to the trigger struct for those reactions.
    pingpong_pong_0_0[0] = &pingpong_self->__pong.send_trigger;;
    ...
    // Connect PortInstance PingPong.pong.receive, which gets data from reaction 1
    // of PingPong, to PingPong.pong.receive.
    pingpong_pong_self->__receive = (pong_receive_t*)&pingpong_self->__pong.receive;
    // Record output PingPong.pong.send, which triggers reaction 2
    // of PingPong, on its self struct.
    pingpong_self->__pong.send = (pong_send_t*)&pingpong_pong_self->__send;
    // END Connect inputs and outputs for reactor PingPong.
    // Add port PingPong.pong.receive to array of is_present fields.
    __is_present_fields[0] 
            = &pingpong_self->__pong.receive.
            is_present;
    // Add port PingPong.pong.send to array of is_present fields.
    __is_present_fields[1] = &pingpong_pong_self->__send.is_present
    ;
    ...
    pingpong_self->___reaction_1.chain_id = 3;
    // index is the OR of level 0 and 
    // deadline 140737488355327 shifted left 16 bits.
    pingpong_self->___reaction_1.index = 0x7fffffffffff0000LL;
    pingpong_self->___reaction_2.chain_id = 1;
    // index is the OR of level 2 and 
    // deadline 140737488355327 shifted left 16 bits.
    pingpong_self->___reaction_2.index = 0x7fffffffffff0002LL;
    pingpong_pong_self->___reaction_0.chain_id = 3;
    // index is the OR of level 1 and 
    // deadline 140737488355327 shifted left 16 bits.
    pingpong_pong_self->___reaction_0.index = 0x7fffffffffff0001LL;
    pingpong_pong_self->___reaction_1.chain_id = 3;
    // index is the OR of level 2 and 
    // deadline 140737488355327 shifted left 16 bits.
